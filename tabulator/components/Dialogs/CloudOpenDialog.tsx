...
import { X, Cloud, FileSpreadsheet, Search, Trash2 } from 'lucide-react'
import { useAuth } from '@shared/contexts/AuthContext'
import { listCloudSpreadsheets, deleteCloudSpreadsheet, type CloudSpreadsheetMeta } from '@/lib/cloud/cloudSpreadsheetService'
...
export function CloudOpenDialog({ onOpen, onClose }: CloudOpenDialogProps) {
  ...
  const [documents, setDocuments] = useState<CloudSpreadsheetMeta[]>([])
  ...
  const loadDocuments = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await listCloudSpreadsheets(apiClient, {
  ...
  const handleDelete = useCallback(async (docId: string) => {
    ...
    const success = await deleteCloudSpreadsheet(apiClient, docId)
    if (success) {
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          ...
          {!isLoading && documents.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px', color: '#999', fontSize: '14px' }}>
              {searchQuery ? 'Keine Tabellen gefunden' : 'Noch keine Cloud-Tabellen vorhanden'}
            </div>
          )}
